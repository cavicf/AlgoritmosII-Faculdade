...
#A abordagem utilizada abaixo é uma técnica de endereçamento fechado — Encadeamento(cada slot da tabela possui uma lista encadeada) —  e pode não ser a mais adequada em todos os casos. Isso porque, na análise de pior caso, onde todas as chaves são direcionadas ao mesmo indice e a lista encadeada daquele slot possuir todos os dados acumulados nela, fará com que as operações de  busca e remoção tenham uma complexidade de O(n), pois teremos que percorrer posição a posição para finalizar uma operação. E em um análise de caso médio, a busca do slot da tabela terá uma complexidade O(1), mas a busca dentro do slot, na lista encadeada do mesmo, ainda terá uma complexidade O(n) dependendo da quantidade de dados que foram inseridos nela.
...
class listaLigada:
    ...
    def insereLista(self, chave, valor):
        no = Node(chave, valor)
        #se o meu ponteiro do fim está apontando para alguma coisa, quer dizer q aquela lista ja possui nós, então tenho que inserir meu novo no ao fim da lista
        if self.fim != None:
            self.fim.proximo = no
            self.fim = no #e faço o meu novo nó inserido ser o fim
        else: #agora se o meu ponteiro fim está apontando pra None, quer dizer que a lista está vazia, portanto meu novo nó a ser inserido será o primeiro da lista, sendo então o começo e fim dela
            self.comeco = no
            self.fim = no

    def imprimeLista(self):
        #para imprimir todos os nós da lista, preciso de um auxiliar que começe no inicio da minha lista, para que eu não perca o meu ponteiro marcador do começo
        noAtual = self.comeco
        while noAtual != None:
            print(f'{noAtual.chave}: {noAtual.valor}')
            noAtual = noAtual.proximo
    
    def procurarLista(self, chave):
        noAtual = self.comeco
        while noAtual != None:
            if noAtual.chave == chave:
                print(f'{chave} está na lista e vale:')
    ...
    def removerLista(self, chave):
        noAtual = self.comeco
        anterior = None
        while noAtual != None:
            if noAtual.chave == chave:
                if anterior == None:
                    self.comeco = noAtual.proximo
            ...
            noAtual = noAtual.proximo
        if noAtual == None:
            print(f'{chave} ja não existia na lista')
            return None
...
class HashTable:
    def __init__(self, tamanho):
        ...
        self.tabela = [listaLigada() for slot in range(tamanho)]
        self.qtdItens = 0
    ...
    def inserirTabela(self, chave, valor):
        ...
        #inserindo o dado na lista encadeada presente no indice fornecido da função hash
        self.tabela[indice].insereLista(chave, valor)
        self.qtdItens += 1

    
    def procurarTabela(self, chave):
        indice = self.funcaoHash(chave)
        #buscando uma chave na lista encadeada presente no indice fornecido da função hash
        return self.tabela[indice].procurarLista(chave)

    def removerTabela(self, chave):
        indice = self.funcaoHash(chave)
        #renovendo uma chave na lista encadeada presente no indice fornecido da função hash
        self.tabela[indice].removerLista(chave)
...
#----------------------------------------------------------------------------------------------------------------------------------
#Testando a tabela hash com lista encadeada pra resolver colisões
