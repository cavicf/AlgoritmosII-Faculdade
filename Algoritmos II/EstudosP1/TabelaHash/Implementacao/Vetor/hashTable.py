...
#A abordagem utilizada abaixo é uma técnica de endereçamento fechado — Vetor(cada slot da tabela possui um vetor) —  e pode não ser a mais adequada em todos os casos. Isso porque, na análise de pior caso, onde todas as chaves são direcionadas ao mesmo indice e o vetor daquele slot possuir todos os dados acumulados nele, fará com que as operações de  busca tenham uma complexidade de O(n), pois teremos que percorrer posição a posição para finalizar uma operação. E em um análise de caso médio, a busca do slot da tabela terá uma complexidade O(1), mas a busca dentro do slot, no vetor do mesmo, ainda terá uma complexidade O(n) dependendo da quantidade de dados que foram inseridos nela.
class HashItem:
...
class HashTable:
    def __init__(self, tamanho):
        ...
        self.tabela = [[] for slot in range(self.tamanho)]
        self.qtdItens = 0
